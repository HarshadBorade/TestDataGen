...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["C", "U", "PA", "PR", "A"],
        required=True,
        help="C=Customers, U=Users, PA=Parts, PR=Products, A=All",
    )
    parser.add_argument(
        "--multi",
        "-M",
        action="store_true",
        help="For WRITE mode: create multiple records",
    )
    parser.add_argument(
        "--count",
        "-c",
        type=int,
        default=1,
        help="Number of records to create when using --multi (default: 1)",
    )
